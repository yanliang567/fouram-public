...
class ApiCollectionWrapper:
    ...
    def init_collection(self, name, schema=None, using="default", check_task=None, check_items=None, **kwargs):
        ...
        res = api_request([Collection, name, schema, using], **kwargs)
        self.collection = res.response if res.res_result else None
        check_result = ResponseChecker(res, func_name, check_task, check_items, name=name, schema=schema, using=using,
                                       **kwargs).run()
    ...
    def construct_from_dataframe(self, name, dataframe, check_task=None, check_items=None, **kwargs):
        func_name = sys._getframe().f_code.co_name
        res = api_request([Collection.construct_from_dataframe, name, dataframe], **kwargs)
        self.collection = res.response if res.res_result else None
        check_result = ResponseChecker(res, func_name, check_task, check_items, name=name, dataframe=dataframe,
                                       **kwargs).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def drop(self, check_task=None, check_items=None, **kwargs):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.drop], **kwargs)
    ...
    def set_properties(self, properties, timeout=None, check_task=None, check_items=None, **kwargs):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.set_properties, properties, timeout], **kwargs)
        check_result = ResponseChecker(res, func_name, check_task, check_items, properties=properties, **kwargs).run()
        return InterfaceResponse(*res.get_res_list, check_result)
    ...
    def load(self, partition_names=None, replica_number=NaN, timeout=None, check_task=None, check_items=None, **kwargs):
        ...
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.load, partition_names, replica_number, timeout], **kwargs)
        check_result = ResponseChecker(res, func_name, check_task, check_items, partition_names=partition_names,
                                       **kwargs).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def release(self, check_task=None, check_items=None, **kwargs):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.release], **kwargs)
        check_result = ResponseChecker(res, func_name, check_task, check_items, **kwargs).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def insert(self, data, partition_name=None, check_task=None, check_items=None, **kwargs):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.insert, data, partition_name], **kwargs)
        check_result = ResponseChecker(res, func_name, check_task, check_items, dat=data, partition_name=partition_name,
                                       **kwargs).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def upsert(self, data, partition_name=None, timeout=None, check_task=None, check_items=None, **kwargs):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.upsert, data, partition_name, timeout], **kwargs)
        check_result = ResponseChecker(res, func_name, check_task, check_items, dat=data, partition_name=partition_name,
                                       **kwargs).run()
        return InterfaceResponse(*res.get_res_list, check_result)
    ...
    def flush(self, check_task=None, check_items=None, **kwargs):
        ...
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.flush], **kwargs)
        check_result = ResponseChecker(res, func_name, check_task, check_items, **kwargs).run()
        return InterfaceResponse(*res.get_res_list, check_result)
    ...
    def search(self, data, anns_field, param, limit, expr=None, partition_names=None, output_fields=None, timeout=None,
        ...
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.search, data, anns_field, param, limit, expr, partition_names,
        ...
        check_result = ResponseChecker(res, func_name, check_task, check_items, data=data, anns_field=anns_field,
                                       param=param, limit=limit, expr=expr, partition_names=partition_names,
                                       output_fields=output_fields, timeout=timeout, **kwargs).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def hybrid_search(self, reqs, rerank, limit, partition_names=None, output_fields=None, timeout=None,
                      round_decimal=-1, check_task=None, check_items=None, **kwargs):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.hybrid_search, reqs, rerank, limit, partition_names, output_fields, timeout,
    ...
    def query(self, expr, output_fields=None, partition_names=None, timeout=None, check_task=None, check_items=None,
              **kwargs):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.query, expr, output_fields, partition_names, timeout], **kwargs)
    ...
    def partition(self, partition_name, check_task=None, check_items=None):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.partition, partition_name])
        check_result = ResponseChecker(res, func_name, check_task, check_items, partition_name=partition_name).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def has_partition(self, partition_name, check_task=None, check_items=None):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.has_partition, partition_name])
        check_result = ResponseChecker(res, func_name, check_task, check_items, partition_name=partition_name).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def drop_partition(self, partition_name, check_task=None, check_items=None, **kwargs):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.drop_partition, partition_name], **kwargs)
        check_result = ResponseChecker(res, func_name, check_task, check_items, partition_name=partition_name,
                                       **kwargs).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def create_partition(self, partition_name, check_task=None, check_items=None, description="", **kwargs):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.create_partition, partition_name, description], **kwargs)
        check_result = ResponseChecker(res, func_name, check_task, check_items, partition_name=partition_name,
                                       **kwargs).run()
        return InterfaceResponse(*res.get_res_list, check_result)
    ...
    def index(self, check_task=None, check_items=None):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.index])
    ...
    def create_index(self, field_name, index_params, check_task=None, check_items=None, **kwargs):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.create_index, field_name, index_params], **kwargs)
        check_result = ResponseChecker(res, func_name, check_task, check_items, field_name=field_name,
    ...
    def alter_index(self, index_name, extra_params, timeout=None, check_task=None, check_items=None):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.alter_index, index_name, extra_params, timeout])
        check_result = ResponseChecker(res, func_name, check_task, check_items, index_name=index_name,
                                       extra_params=extra_params, timeout=timeout).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def has_index(self, check_task=None, check_items=None, **kwargs):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.has_index], **kwargs)
        check_result = ResponseChecker(res, func_name, check_task, check_items, **kwargs).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def drop_index(self, check_task=None, check_items=None, **kwargs):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.drop_index], **kwargs)
        check_result = ResponseChecker(res, func_name, check_task, check_items, **kwargs).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def create_alias(self, alias_name, check_task=None, check_items=None, **kwargs):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.create_alias, alias_name], **kwargs)
        check_result = ResponseChecker(res, func_name, check_task, check_items, **kwargs).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def drop_alias(self, alias_name, check_task=None, check_items=None, **kwargs):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.drop_alias, alias_name], **kwargs)
        check_result = ResponseChecker(res, func_name, check_task, check_items, **kwargs).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def alter_alias(self, alias_name, check_task=None, check_items=None, **kwargs):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.alter_alias, alias_name], **kwargs)
        check_result = ResponseChecker(res, func_name, check_task, check_items, **kwargs).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def delete(self, expr, partition_name=None, timeout=None, check_task=None, check_items=None, **kwargs):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.delete, expr, partition_name, timeout], **kwargs)
        check_result = ResponseChecker(res, func_name, check_task, check_items, **kwargs).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def compact(self, timeout=None, check_task=None, check_items=None, **kwargs):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.compact, timeout], **kwargs)
        check_result = ResponseChecker(res, func_name, check_task, check_items, **kwargs).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def get_compaction_state(self, timeout=None, check_task=None, check_items=None, **kwargs):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.get_compaction_state, timeout], **kwargs)
        check_result = ResponseChecker(res, func_name, check_task, check_items, **kwargs).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def get_compaction_plans(self, timeout=None, check_task=None, check_items={}, **kwargs):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.get_compaction_plans, timeout], **kwargs)
        check_result = ResponseChecker(res, func_name, check_task, check_items, **kwargs).run()
        return InterfaceResponse(*res.get_res_list, check_result)
    ...
    def get_replicas(self, timeout=None, check_task=None, check_items=None, **kwargs):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.get_replicas, timeout], **kwargs)
        check_result = ResponseChecker(res, func_name, check_task, check_items, **kwargs).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def describe(self, timeout=None, check_task=None, check_items=None, **kwargs):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.collection.describe, timeout], **kwargs)
        check_result = ResponseChecker(res, func_name, check_task, check_items, **kwargs).run()
        return InterfaceResponse(*res.get_res_list, check_result)
