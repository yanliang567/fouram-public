...
class ApiIndexWrapper:
    ...
    def init_index(self, collection, field_name, index_params, check_task=None, check_items=None, **kwargs):
        ...
        res = api_request([Index, collection, field_name, index_params], **kwargs)
        self.index = res.response if res.res_result else None
        check_result = ResponseChecker(res, func_name, check_task, check_items, collection=collection,
    ...
    def drop(self, check_task=None, check_items=None, **kwargs):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.index.drop], **kwargs)
