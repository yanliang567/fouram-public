...
class ApiRoleWrapper:
    ...
    def init_role(self, name, using="default", check_task=None, check_items=None, **kwargs):
        ...
        res = api_request([Role, name, using], **kwargs)
        self.role = res.response if res.res_result else None
    ...
    def create(self, check_task=None, check_items=None):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.role.create])
        check_result = ResponseChecker(res, func_name, check_task, check_items).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def drop(self, check_task=None, check_items=None):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.role.drop])
        check_result = ResponseChecker(res, func_name, check_task, check_items).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def add_user(self, username, check_task=None, check_items=None):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.role.add_user, username])
        check_result = ResponseChecker(res, func_name, check_task, check_items, username=username).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def remove_user(self, username, check_task=None, check_items=None):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.role.remove_user, username])
        check_result = ResponseChecker(res, func_name, check_task, check_items, username=username).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def get_users(self, check_task=None, check_items=None):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.role.get_users])
        check_result = ResponseChecker(res, func_name, check_task, check_items).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def is_exist(self, check_task=None, check_items=None):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.role.is_exist])
        check_result = ResponseChecker(res, func_name, check_task, check_items).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def grant(self, object, object_name, privilege, check_task=None, check_items=None):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.role.grant, object, object_name, privilege])
        check_result = ResponseChecker(res, func_name, check_task, check_items, object=object, object_name=object_name,
                                       privilege=privilege).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def revoke(self, object, object_name, privilege, check_task=None, check_items=None):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.role.revoke, object, object_name, privilege])
        check_result = ResponseChecker(res, func_name, check_task, check_items, object=object, object_name=object_name,
                                       privilege=privilege).run()
        return InterfaceResponse(*res.get_res_list, check_result)
    ...
    def list_grant(self, object, object_name, check_task=None, check_items=None):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.role.list_grant, object, object_name])
        check_result = ResponseChecker(res, func_name, check_task, check_items, object=object,
                                       object_name=object_name).run()
        return InterfaceResponse(*res.get_res_list, check_result)

    def list_grants(self, check_task=None, check_items=None):
        func_name = sys._getframe().f_code.co_name
        res = api_request([self.role.list_grant])
        check_result = ResponseChecker(res, func_name, check_task, check_items).run()
        return InterfaceResponse(*res.get_res_list, check_result)
