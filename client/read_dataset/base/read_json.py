...
from client.common.common_func import read_json_strings_file
...
class ReadJson(ReadBase):
    ...
    def get_data(self, data_length: int):
        log.debug(f"[ReadJson] Get data from dataset `{self._dataset_name}`, length: {data_length}")

        while len(self._default_value) < data_length:
            self._default_value.extend(read_json_strings_file(self.file_name))
    ...
    def read_specified_file(file_name: str, **kwargs) -> (any, int):
        _file_data = read_json_strings_file(file_name)
