...
class AlgorithmBase:
    ...
    def parser_json_repeat(self, default_value: int = None, __max_value: int = 10000, **kwargs):
        name = "json_repeat"
        _json_repeat = kwargs.get(name, default_value)
        if _json_repeat is not None and not (
                isinstance(_json_repeat, int) and 1 <= _json_repeat <= __max_value):
            raise ValueError("[{0}] Parser `{1}` failed: {2}, `{1}` must be an integer: 1 <= {1} <= {3}".format(
                self._class_name, name, _json_repeat, __max_value))

        log.debug(f"[{self._class_name}] Parser `{name}` value: {_json_repeat}")
        return _json_repeat
...
class AlgorithmMixedValuesJson(AlgorithmBase):
    def __init__(self, field_name: str, field_dtype: DataType, **kwargs):
        ...
        Params:
            ...
            json_repeat: int, default value: None
                        - None: generate single json data according to `json_key` and `json_depth`
                        - int: 1<= json_repeat <= 10000, copy the data generated by `json_key` and `json_depth` into a set of json value
                            - e.g. json_repeat=2: {"key_0": {'k1': {'k2': {'k3': `value`}}},
                                                   "key_1": {'k1': {'k2': {'k3': `value`}}}}
        ...
        self._json_repeat = self.parser_json_repeat(**kwargs)
    ...
    def _gen_default_base_value(self, json_dict: dict, base_value: list):
        _b = [set_dict_value(json_dict, d, {}) for d in base_value]
        if self._json_repeat is None:
            return _b
        return [{f"key_{i}": v for i in range(self._json_repeat)} for v in _b]
